...
import { EmailAuthProvider, isSignInWithEmailLink, linkWithCredential, sendSignInLinkToEmail, signInAnonymously, signInWithEmailLink, signOut, type User } from "firebase/auth";
...
export async function betaAuthStatus(){const fb=getLookGoFirebase();if(!fb)return {ready:false,durable:false,email:""};const user=await cloudUser();return {ready:Boolean(user),durable:Boolean(user&&!user.isAnonymous),email:user?.email||""}}
