...
export default function AnalyseStep(){
 ...
 async function generateVideo(tier:Tier){
  ...
  try{
   const form=new FormData();
   form.append("image",new File([imageBlob],`${tier}-tryon.png`,{type:imageBlob.type||"image/png"}));
   form.append("tier",tier);
   const create=await fetch("/api/ai/runway",{method:"POST",body:form});
   const created=await create.json();
   if(!create.ok) throw new Error(created.error||"Création du défilé impossible");
   ...
   for(let attempt=0;attempt<48;attempt++){
    ...
    if(!statusRes.ok) throw new Error(status.error||"Suivi du défilé impossible");
    setVideos(v=>({...v,[tier]:{...v[tier],status:"loading",progress:Number(status.progress||0)}}));
    if(status.status==="failed") throw new Error(status.error||"La génération vidéo a échoué");
   ...
   if(!content.ok){const data=await content.json().catch(()=>({}));throw new Error(data.error||"Impossible de récupérer la vidéo");}
