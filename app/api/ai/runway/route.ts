...
function runwayPrompt(tier:string){const label=tier==="signature"?"Signature":tier==="smart"?"Smart":"Équilibre";return [`Create a realistic vertical fashion runway clip for the ${label} Look&Go look using the supplied image as the exact visual reference and starting frame.`,"Preserve the same adult person's facial identity, skin tone, hairstyle, apparent age, body proportions, morphology and outfit exactly as shown in the reference image.","Do not beautify, slim, enlarge, reshape, de-age or replace the person. Do not redesign, recolor or swap the clothing.","The source image already contains the official Look&Go brand mark. Preserve that existing mark as a stable, subtle lower-corner element throughout the clip. Do not redraw, restyle or replace it, and do not add other text.","Motion should be restrained and physically natural: a slow confident walk forward, subtle fabric movement, then a gentle 30 to 45 degree turn.","Keep the full body visible from head to shoes. Stable premium editorial lighting, clean fashion-studio background, realistic anatomy, no camera warping, no sudden cuts.","No dialogue or captions. Do not add any new text overlays. The clip is a fashion visualization. Prioritize identity, outfit and existing brand-mark continuity over dramatic motion."].join(" ")}
...
async function createGoogle(image:File,tier:string){
 ...
 const response=await timedFetch(`${GOOGLE_BASE}/models/${encodeURIComponent(model)}:predictLongRunning`,{method:"POST",headers:{"x-goog-api-key":key,"content-type":"application/json"},body:JSON.stringify({instances:[{prompt:runwayPrompt(tier),image:{inlineData:{mimeType:image.type,data}}}],parameters:{aspectRatio:"9:16",durationSeconds:"8",resolution:"720p",personGeneration:"allow_adult"}})});
 const body=await response.json().catch(()=>({}));
 if(!response.ok){console.error("Google Veo create failed",response.status,JSON.stringify(body).slice(0,1200));if(response.status===429)throw new Error("GOOGLE_QUOTA");if(response.status===401||response.status===403)throw new Error("GOOGLE_AUTH");throw new Error(`GOOGLE_HTTP_${response.status}`)}
 const name=String(body?.name||"");if(!name)throw new Error("GOOGLE_NO_OPERATION");
