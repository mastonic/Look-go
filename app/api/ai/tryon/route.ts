...
function weddingRoleDirection(role:string,whiteAllowed:boolean){
 if(role==="bride")return "The customer is the bride. Bridal white, ivory and modern bridal alternatives are allowed. Create sophisticated bridal fashion, never costume styling, and keep the outfit credible for the stated venue and dress code.";
...
export async function POST(request:Request){
 ...
 try{
  ...
  const portrait=incoming.get("portrait");const fullBody=incoming.get("fullBody");const tier=String(incoming.get("tier")||"").toLowerCase();const profile=String(incoming.get("profile")||"{}");
  ...
  const weddingContext=weddingEnabled?[
   ...
   eventColors.length?`Wedding palette requested by the customer: ${eventColors.join(", ")}.`:"No mandatory wedding palette was specified.",
   eventAvoid.length?`Wedding-specific colors to avoid: ${eventAvoid.join(", ")}.`:"No extra wedding-specific avoided colors were specified.",
  ...
  const prompt=[
   ...
   `Create a ${tierDirection(tier)}. Preferred styles: ${styles.join(", ")||"open contemporary fashion"}. Preferred colors: ${effectiveColors.join(", ")||"harmonious colors"}. Avoid: ${effectiveAvoid.join(", ")||"none"}.`,
  ...
  for(const provider of providers){
   ...
   try{
    ...
    if(provider.id==="google")result=await generateWithGoogle(prompt,portrait,fullBody);else if(provider.id==="openai")result=await generateWithOpenAI(prompt,portrait,fullBody);else continue;
    if(result){const meta=buildAiCostMeta(result.provider,result.model,"image",Date.now()-started);console.info("TRYON_SUCCESS",JSON.stringify({tier,silhouette,trendPreference,trendBoldness,wedding,weddingRole:wedding?weddingRole:null,...meta,totalDurationMs:Date.now()-requestStarted}));return NextResponse.json({...result,tier,silhouette,trendPreference,trendBoldness,wedding,weddingRole:wedding?weddingRole:null,meta})}
