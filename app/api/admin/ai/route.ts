...
async function firebaseAdminEmail(request:Request){
 ...
 return String(body?.users?.[0]?.email||"").trim().toLowerCase();
}

async function isAdmin(request:Request){
 const session=await auth();
 const nextAuthEmail=String(session?.user?.email||"").trim().toLowerCase();
 if(nextAuthEmail===ADMIN_EMAIL)return true;
 return (await firebaseAdminEmail(request))===ADMIN_EMAIL;
}
...
export async function POST(request:Request){
 ...
 if(body?.action==="test"){
  try{return NextResponse.json({ok:true,message:await testProvider(provider,apiKey||undefined)});}catch(error){return NextResponse.json({error:error instanceof Error?error.message:"Connexion impossible."},{status:502});}
 }
 ...
 try{
  ...
  return NextResponse.json({ok:true,message:apiKey?"Nouvelle clé validée et enregistrée côté serveur. Redéploiement de production lancé.":"Configuration enregistrée. Redéploiement de production lancé.",deployment});
 }catch(error){
