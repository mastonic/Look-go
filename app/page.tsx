...
export default function HomePage() {
  return <main>
    <header className="site-header"><Link href="#top" className="wordmark">LOOK&GO</Link><nav className="desktop-nav"><Link href="#shopping">3 budgets</Link><Link href="#try-on">Try-On</Link><Link href="#styliste">Styliste</Link><Link href="#offres">Offres</Link></nav><div className="header-actions"><Link href="#connexion" className="text-link">Connexion</Link><Link href="#commencer" className="button button-small button-dark">Créer mon dressing</Link></div></header>
    ...
    <section id="try-on" className="section section-shell split-section colorful-split"><div className="section-kicker">TRY-ON</div><div className="section-copy-large"><h2>Voyez le look.<br/><em>Avant de l’acheter.</em></h2><p>Le Try-On devient la passerelle entre la recommandation et l’achat : Signature, Équilibre ou Smart, comparez d’abord sur vous.</p><Link href="#commencer" className="button button-coral">Voir sur moi</Link></div><div className="editorial-image editorial-image-tall image-frame-coral"><Image src={editorialImages.vibrantWoman} alt="Portrait mode naturel d’une femme noire portant une tenue colorée" fill sizes="(max-width:768px) 100vw,42vw"/></div></section>
    ...
    <section id="commencer" className="final-cta"><div className="final-orb final-orb-one"/><div className="final-orb final-orb-two"/><div className="section-shell final-cta-inner"><p className="eyebrow">LOOK&GO</p><h2>Votre look.<br/><em>Votre budget.</em></h2><p>Commencez par votre profil. Look&Go s’occupe de réduire les choix.</p><Link href="#connexion" className="button button-light">Créer mon dressing privé</Link><span>Les achats réels seront activés marchand par marchand après connexion des catalogues autorisés.</span></div></section>

    <footer id="connexion" className="site-footer section-shell"><div><div className="wordmark footer-wordmark">LOOK&GO</div><p>Your style. Your fit. Your choice.</p></div><nav><Link href="#shopping">3 budgets</Link><Link href="#try-on">Try-On</Link><Link href="#offres">Offres</Link></nav><p className="footer-note">Look&Go distingue explicitement les démonstrations UI des fonctionnalités connectées. Aucun partenariat, prix, stock ou commission d’affiliation n’est inventé.</p></footer>
  </main>;
}
