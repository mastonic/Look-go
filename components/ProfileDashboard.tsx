...
async function mediaBlob(key: BetaMediaKey) {
  try {
    const local = await readBetaMedia(key);
    if (local) return local;
  } catch {}
  try {
    return (await readBetaMediaCloud(key))?.blob || null;
  } catch {
    return null;
  }
}
...
function itemLabel(item: WardrobeItem) {
  return [item.garmentType || item.subcategory || item.categoryLabel, item.primaryColor].filter(Boolean).join(" · ");
}

function itemScore(item: WardrobeItem, profile: BetaProfile) {
  const likedColors = (profile.likedColors || []).map(norm);
  const avoided = (profile.avoidColors || []).map(norm);
  const likedStyles = (profile.styles || []).map(norm);
  const occasions = (profile.occasions || []).map(norm);
  const color = norm(item.primaryColor);
  const itemStyles = (item.styles || []).map(norm);
  const itemOccasions = (item.occasions || []).map(norm);
...
function pick(items: WardrobeItem[], categories: WardrobeCategory[], profile: BetaProfile, used: Set<string>) {
  return (
    items
      .filter((item) => categories.includes(item.category) && !used.has(item.id) && itemScore(item, profile) > -50)
      .sort((a, b) => itemScore(b, profile) - itemScore(a, profile))[0] || null
  );
}
...
function buildWardrobeLooks(items: WardrobeItem[], profile: BetaProfile): WardrobeLook[] {
  ...
  const specs: Array<{ title: string; reason: string; groups: WardrobeCategory[][] }> = [
    {
      title: "Facile aujourd’hui",
      reason: "Une proposition portable construite à partir de vos goûts et couleurs autorisées.",
      groups: [["tops", "knitwear"], ["bottoms"], ["shoes"], ["outerwear", "bags", "accessories"]],
    },
    {
      title: "Plus habillé",
      reason: "Une silhouette plus élégante sans sortir des pièces réellement présentes dans votre dressing.",
      groups: [["dresses", "jumpsuits", "sets"], ["shoes"], ["outerwear", "bags", "jewelry", "accessories"]],
    },
    {
      title: "Nouvelle association",
      reason: "Look&Go rapproche des pièces compatibles avec votre profil pour vous faire redécouvrir votre penderie.",
      groups: [["tops", "knitwear"], ["bottoms"], ["outerwear"], ["shoes", "bags", "accessories"]],
    },
  ];
  ...
  for (const spec of specs) {
    ...
    for (const group of spec.groups) {
      const candidate = pick(items, group, profile, used);
      if (candidate) {
        selected.push(candidate);
        used.add(candidate.id);
      }
    }
    if (selected.length >= 2) looks.push({ ...spec, items: selected });
  }
...
function historyLabel(item: HistoryItem) {
  ...
  if (kind === "tryon")
    return `${mode === "wedding" ? "Look mariage" : "Look IA"}${tier ? ` · ${tier}` : ""}${action === "recolor" ? " · couleur modifiée" : action === "multi-angle" ? " · nouvel angle" : ""}`;
...
function historyMeta(item: HistoryItem) {
  const color = String(item.primaryColor || "");
  const angle = String(item.angle || "");
...
export default function ProfileDashboard() {
  ...
  useEffect(() => {
    ...
    const urls: string[] = [];
    void (async () => {
      const p = readBetaProfile();
      if (alive) setProfile(p);

      const [items, wardrobeItems] = await Promise.all([readBetaHistoryCloud(8), readWardrobeItems(120)]);
      if (alive) {
        setHistory(items);
      ...
      for (const item of ordered) {
        const blob = await mediaBlob(item.key);
        if (blob) {
          const url = URL.createObjectURL(blob);
          urls.push(url);
          if (alive) setLatest({ key: item.key, url, label: item.label, href: item.href });
          break;
        }
      }
      ...
      for (const item of [
        { key: "portrait" as BetaMediaKey, label: "Portrait" },
        { key: "fullBody" as BetaMediaKey, label: "Plein pied" },
      ]) {
        const blob = await mediaBlob(item.key);
        if (blob) {
          const url = URL.createObjectURL(blob);
          urls.push(url);
          refResults.push({ label: item.label, url });
        }
      }
  ...
  return (
    <section className={styles.dashboard} aria-label="Tableau de bord Look&Go">
      <div className={styles.welcome}>
        <div>
          <span>VOTRE DRESSING PRIVÉ</span>
          <h2>Bonjour, <em>{firstName}</em></h2>
          <p>Votre style, vos dernières créations et tous vos raccourcis au même endroit.</p>
        </div>
      ...
      <div className={styles.heroGrid}>
        <article className={styles.latestCard}>
          ...
          <div className={styles.latestVisual}>
            {latest ? <img src={latest.url} alt={`Dernier look ${latest.label}`} /> : <div className={styles.empty}>{loading ? "Ouverture de votre dressing…" : "Votre prochain look apparaîtra ici."}</div>}
          </div>
          <div className={styles.latestFoot}>
            <div>
              <strong>{latest ? "Votre dernier Try-On" : "Commencez votre look-book"}</strong>
              <small>{latest ? "Retrouvez-le, régénérez-le ou créez son défilé." : "Signature · Équilibre · Smart"}</small>
            </div>
            <Link href={latest?.href || "/inscription/analyse"}>{latest ? "Ouvrir" : "Créer"} →</Link>
          </div>
        </article>
        ...
        <div className={styles.sideColumn}>
          ...
          <article className={styles.referencePanel}>
            <div><span>MES RÉFÉRENCES</span><strong>Photos utilisées par l’IA</strong></div>
            <div className={styles.refImages}>
              {refs.length ? refs.map((ref) => <figure key={ref.label}><img src={ref.url} alt={ref.label} /><figcaption>{ref.label}</figcaption></figure>) : <small>Ajoutez votre portrait et votre photo plein pied.</small>}
            </div>
            <Link href="/inscription">Gérer mes photos →</Link>
          </article>
        </div>
      </div>
      ...
      <section className={styles.wardrobeIdeas}>
        <div className={styles.sectionHead}>
          <div><span>LOOKS AVEC MON DRESSING</span><h3>À porter avec ce que vous avez déjà</h3></div>
          <Link href="/dressing">Voir mon dressing →</Link>
        </div>
        {wardrobeLooks.length ? (
          <div className={styles.ideaGrid}>
            {wardrobeLooks.map((look, index) => (
              <article className={styles.ideaCard} key={look.title}>
                <div className={styles.ideaTop}><span>0{index + 1}</span><strong>{look.title}</strong></div>
                <p>{look.reason}</p>
                <div className={styles.pieces}>{look.items.map((item) => <span key={item.id}>{itemLabel(item)}</span>)}</div>
                <small>Basé uniquement sur les pièces enregistrées et vos préférences Look&Go.</small>
              </article>
            ))}
          </div>
        ) : (
          <div className={styles.wardrobeEmpty}>
            <div><strong>Votre dressing peut devenir votre styliste quotidien.</strong><p>Scannez quelques pièces : Look&Go créera ici des associations à partir de vos vrais vêtements, sans inventer de pièce absente.</p></div>
            <Link href="/dressing">Scanner mon dressing →</Link>
          </div>
        )}
      </section>
      ...
      <div className={styles.lowerGrid}>
        <section className={styles.activity}>
          <div className={styles.sectionHead}>
            <div><span>HISTORIQUE RÉCENT</span><h3>Votre activité</h3></div>
            <Link href="/inscription/analyse">Voir mes looks →</Link>
          </div>
          <div className={styles.timeline}>
            {history.length ? history.slice(0, 5).map((item, index) => (
              <article key={String(item.id || index)}>
                <span>{String(index + 1).padStart(2, "0")}</span>
                <div><strong>{historyLabel(item)}</strong><small>{historyMeta(item)}</small></div>
                <i>{item.success === false ? "!" : "✓"}</i>
              </article>
            )) : <div className={styles.noHistory}>Vos générations et actions récentes apparaîtront ici automatiquement.</div>}
          </div>
        </section>
